...
class DataLogger(Node):
    ...
    def save_to_db(self):
        if len(self.data_buffer) >= 100000:
            conn = sqlite3.connect('ros_data.db')
            cursor = conn.cursor()
            cursor.executemany("INSERT INTO messages (temp, humid, qualite) VALUES (?, ?, ?)", self.data_buffer)
            conn.commit()
            conn.close()
            self.data_buffer = []
    ...
    def check_and_store(self):
        if self.latest_temp is not None and self.latest_humid is not None and self.latest_qualite is not None:
            self.data_buffer.append((self.latest_temp, self.latest_humid, self.latest_qualite))
            self.get_logger().info(f"Stockage en mémoire tampon : {self.latest_temp}, {self.latest_humid}, {self.latest_qualite}")
