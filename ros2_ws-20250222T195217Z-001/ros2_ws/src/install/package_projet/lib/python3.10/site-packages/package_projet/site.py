...
class MinimalPublisher(Node):
    def __init__(self):
        ...
        self.publisher_bdd = self.create_publisher(String, 'bdd', 10)
    ...
    def send_bdd(self, msg_text):
        ...
        msg = String()
        msg.data = "enregistre"
        self.publisher_bdd.publish(msg)
        self.get_logger().info(f"Message envoyé: {msg_text}")
        print("envoi")
...
@app.route('/send_bdd', methods=['POST'])
def sendbdd():
    global publisher_node
    publisher_node.send_bdd("enregistre")
    return jsonify({"message": "Données enregistrées dans la base"})
